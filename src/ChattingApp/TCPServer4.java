...
public class TCPServer4 {
	...
	{
		try {
			welcomeSocket = new ServerSocket(6004);
		} catch (IOException e) {
			e.printStackTrace();
		}
		System.out.println("Server 4 is open.");
		Thread t = new Thread()
		{
			public void run()
			{
							...
							else
							{
								...
								else
								{
									...
									if(first.equals("find"))
									{
										...
										// destination client at server 2
		...
		while(true)
		{
			try {
				...
				users.put(sourceName,connectionSocket);
				new Thread(new Server4Helper(connectionSocket,sourceName)).start();
	...
	public void connectToServer1()
	{
		try {
			...
			outToServer1.writeBytes("Server 4" + '\n');
		} catch (IOException e) {
			connectToServer1();
		}
	}
